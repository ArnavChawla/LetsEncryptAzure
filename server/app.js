...
app.get('/generate/:tagId', function(req, res) {
    ls = spawn('~/.acme.sh/acme.sh',['--issue' , '--debug','-d', `${req.params.tagId}`,'--dns','--yes-I-know-dns-manual-mode-enough-go-ahead-please']);
...
{
    var generated = false
    ls = spawn('~/.acme.sh/acme.sh',['--renew' , '--debug','-d', `${req.params.tagId}`,'--yes-I-know-dns-manual-mode-enough-go-ahead-please']);
    ls.stdout.on("data", data => {
        console.log(`stdout: ${data}`);
    ...
    ls.on("close", code => {
        ...
        if (generated == true){
            ...
            archive.directory(`~/.acme.sh/${req.params.tagId}`, false);
            archive.finalize();
        }
