...
const { spawn } = require("child_process"),options = {stdio: 'inherit'};
...
app.get('/generate/:tagId', function(req, res) {
    ...
    ls = spawn(process.env.HOME+'/.acme.sh/acme.sh',['--issue' , '--debug','-d', `${req.params.tagId}`,'--dns','--yes-I-know-dns-manual-mode-enough-go-ahead-please']);
...
{
    var generated = false
    ls = spawn(process.env.HOME +'/.acme.sh/acme.sh',['--renew' , '--debug','-d', `${req.params.tagId}`,'--yes-I-know-dns-manual-mode-enough-go-ahead-please']);
    ls.stdout.on("data", data => {
        console.log(`stdout: ${data}`);
    ...
    ls.on("close", code => {
        ...
        if (generated == true){
            ...
            archive.directory(process.env.HOME+`/.acme.sh/${req.params.tagId}`, false);
            archive.finalize();
        }
